...
void test(int id_p){
    ...
    if(state_p[id_p]== hungry && state_p[(id_p-1+n_p)%n_p]!=eating && state_p[(id_p+1)%n_p]!= eating){
        state_p[id_p] = eating;
        my_sem_post(&my_sem_p[id_p]); //increment value;
    }
}

void eat(int id_p){
    my_sem_wait(&my_mutex_buffer);//my_mutex_buffer lock
    state_p[id_p]=hungry;

    test(id_p); //philosopher eating

    my_sem_post(&my_mutex_buffer);
    my_sem_post(&my_sem_p[id_p]);
}
void rest(int id_p){
    my_sem_wait(&my_mutex_buffer);
...
int main(int argc, char **argv ) {
    ...
    my_sem_p = (my_sem_t*) malloc(sizeof(my_sem_t) * n_p);
    int* p_index = (int*) malloc(sizeof (int)*n_p);

    //initialise everything
    for(int i =0; i<n_p; i++){
        my_sem_init(&my_sem_p[i], 0);
        state_p[i] = thinking;
        p_index[i] = i;
    }
    my_sem_init(&my_mutex_buffer, 1); //value given as 1 to act as a my_mutex_buffer
